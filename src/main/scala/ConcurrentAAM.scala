...
class ConcurrentAAM[Exp : Expression, Abs : AbstractValue, Addr : Address, Time : Timestamp, TID : ThreadIdentifier]
  ...
  case class Context(control: Control, kstore: KontStore[KontAddr], a: KontAddr, t: Time) {
    def integrate1(tid: TID, a: KontAddr, action: Action[Exp, Abs, Addr])(threads: ThreadMap, results: ThreadResults):
        Set[(ThreadMap, ThreadResults, Store[Addr, Exp, Abs])] = { /* println(s"$control: $action"); printReadWrite(action); */ action match {
      ...
      case ActionError(err) => Set((threads.update(tid, Context(ControlError(err), kstore, a, t)), results, Store.empty[Addr, Exp, Abs]))
      ...
      case ActionJoin(v, σ, _, _) => abs.getTids(v).flatMap(tid2 =>
        if (results.isDone(tid2)) {
          ...
          Set[(ThreadMap, ThreadResults, Store[Addr, Exp, Abs])]()
        })
        }
    }
    ...
    def step(sem: Semantics[Exp, Abs, Addr, Time], tid: TID, store: Store[Addr, Exp, Abs], threads: ThreadMap, results: ThreadResults):
        (Set[(ThreadMap, ThreadResults, Store[Addr, Exp, Abs])]) = control match {
  ...
  case class State(threads: ThreadMap, results: ThreadResults, store: Store[Addr, Exp, Abs]) {
