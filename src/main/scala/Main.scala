...
object Config {
  ...
  object Lattice extends Enumeration {
    type Lattice = Value
    val Concrete, Type, TypeSet, Test, MustTaint = Value
  }
...
object Main {
  ...
  def run[Exp : Expression, Abs : AbstractValue, Addr : Address, Time : Timestamp](machine: AbstractMachine[Exp, Abs, Addr, Time], sem: Semantics[Exp, Abs, Addr, Time])(program: String, output: Option[String]): Unit = {
    ...
    val res2 = s"states=${result.numberOfStates} edges=${result.numberOfEdges} mays=${result.numberOfMays} musts=${result.numberOfMusts} time=${result.time}"
  ...
  def printMemory() = {
    ...
    val heapSize = Runtime.getRuntime().totalMemory() / 100000; 
    
    // Get maximum size of heap in bytes. The heap cannot grow beyond this size.// Any attempt will result in an OutOfMemoryException.
    val heapMaxSize = Runtime.getRuntime().maxMemory() / 1000000;
    
     // Get amount of free memory within the heap in bytes. This size will increase // after garbage collection and decrease as new objects are created.
    val heapFreeSize = Runtime.getRuntime().freeMemory() / 1000000; 
    
    println("Max size ="+heapMaxSize+", Current size ="+heapSize+", FREE="+heapFreeSize+"mb")
  }
...
//          case (false, Config.Machine.ConcurrentAAM, Config.Lattice.TypeSet, false) => run(new ConcurrentAAM[SchemeExp, AbstractTypeSet, ClassicalAddress, ZeroCFA, ConcreteTID], new ConcurrentSchemeSemantics[AbstractTypeSet, ClassicalAddress, ZeroCFA, ConcreteTID]) _
          //TODO: jonas force taint lattice with line below
          //case (false, Config.Machine.AAM, Config.Lattice.Test, false) => run(new AAM[SchemeExp, AbstractTaint, ClassicalAddress, ZeroCFA], new SchemeSemantics[AbstractTaint, ClassicalAddress, ZeroCFA]) _
          /* Example of how to use the product lattice */
          //case (true, Config.Machine.AAM, Config.Lattice.Test, false) => {
//          case (false, Config.Machine.AAM, Config.Lattice.Test, false) => {
//            val prod = new ProductLattice[AbstractTypeSet, AbstractTaint]
//            //ZeroCFA OneCFA
//            import prod._
//            run(new AAM[SchemeExp, Product, ClassicalAddress, OneCFA],
//                new SchemeSemantics[Product, ClassicalAddress, OneCFA]) _
//          }
          case (false, Config.Machine.AAM, Config.Lattice.MustTaint, false) => {
            val prod = new ProductLattice[AbstractTypeSet, AbstractMaybeTaintSource]
            //ZeroCFA OneCFA
            import prod._
            run(new AAM[SchemeExp, Product, ClassicalAddress, ZeroCFA],
                new SchemeSemantics[Product, ClassicalAddress, ZeroCFA]) _
          }
