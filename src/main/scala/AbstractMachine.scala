...
trait Output[Abs] {
  ...
  def numberOfMays: Int
  def numberOfMusts: Int
  def distinctFinalStates: Set[String]
  def shortestPaths: (Double, List[(String, Set[String])])
  /**
...
abstract class EvalKontMachine[Exp : Expression, Abs : AbstractValue, Addr : Address, Time : Timestamp]
  ...
  trait Control {
    ...
    def toString(store: Store[Addr, Exp, Abs]): String = toString()
  }
  ...
  case class ControlEvalEvent(exp: Exp, env: Environment[Addr], event:String) extends Control {
    override def toString() = s"evalevent(${exp})"
    def subsumes(that: Control) = ??? // should never be called as we transform this to a controleval
  }  
  /**
  ...
  case class ControlKont(v: Abs) extends Control {
    ...
    override def toString(store: Store[Addr, Exp, Abs]) = s"ko(${abs.toString(v, store)})"
    def subsumes(that: Control) = that match {
