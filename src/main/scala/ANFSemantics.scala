...
class ANFSemantics[Abs : AbstractValue, Addr : Address, Time : Timestamp]
  ...
  def atomicEval(e: ANFAtomicExp, ρ: Environment[Addr], σ: Store[Addr, ANFExp, Abs]): Either[String, Abs] = e match {
  ...
  def stepEval(e: ANFExp, ρ: Environment[Addr], σ: Store[Addr, ANFExp, Abs], t: Time): Set[Action[ANFExp, Abs, Addr]] = e match {
        ...
        case Right(argsv) =>
          ...
          atomicEval(f, ρ, σ) match {
            ...
            case Right(fv) => {
              ...
              val fromPrim: Set[Action[ANFExp, Abs, Addr]] = abs.getPrimitive[Addr, ANFExp, Abs](fv) match {
  ...
  def stepKont(v: Abs, frame: Frame, σ: Store[Addr, ANFExp, Abs], t: Time) = frame match {
