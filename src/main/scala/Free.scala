...
class Free[Exp : Expression, Abs : AbstractValue, Addr : Address, Time : Timestamp]
  ...
  val initialStore = Store.initial[Addr, Exp, Abs](primitives.forStore)
   ...
   * exploration phase.
   */
  case class State(control: Control, σ: Store[Addr, Exp, Abs], kstore: KontStore[KontAddr], k: KontAddr, t: Time) {
   ...
   */
  case class States(R: Set[Configuration], σ: Store[Addr, Exp, Abs], kstore: KontStore[KontAddr]) {
    ...
    def step(sem: Semantics[Exp, Abs, Addr, Time]): States = {
      ...
      val (σ1, kstore1) = succs.foldLeft((Store.empty[Addr, Exp, Abs], new KontStore[KontAddr]()))((acc, ς) => (acc._1.join(ς.σ), acc._2.join(ς.kstore)))
