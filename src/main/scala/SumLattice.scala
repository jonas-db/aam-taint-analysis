...
class SumLattice[X : AbstractValue, Y : AbstractValue] {
  ...
  trait Sum
  case class Prim[Addr : Address, Exp : Expression, Abs : AbstractValue](prim: Primitive[Addr, Exp, Abs]) extends Sum {
  ...
  implicit object SumAbstractValue extends AbstractValue[Sum] {
    ...
    def objects[Addr : Address](s: Sum) = s match {
      case Left(x) => xabs.objects[Addr](x)
      case Right(y) => yabs.objects[Addr](y)
      case Prim(_) => Set[Addr]()
    }
    def symbols(s: Sum) = s match {
      case Left(x) => xabs.symbols(x)
      case Right(y) => yabs.symbols(y)
      case Prim(_) => Set()
    }   
    def toString[Addr : Address, Exp : Expression](s: Sum, store: Store[Addr, Exp, Sum]) = s.toString
    ...
    def getPrimitive[Addr : Address, Exp : Expression, Abs : AbstractValue](s: Sum) = s match {
      case Prim(p: Primitive[Addr, Exp, Abs]) => Some(p)
      case _ => None
    }
    ...
    def sanitize(s: Sum) = ???
    def sink(s: Sum) = ???
    def taint(s: Sum, sources:Set[Position]) = ???
    def top = Left(xabs.top)
    ...
    def inject[Addr : Address, Exp : Expression, Abs : AbstractValue](x: Primitive[Addr, Exp, Abs]) = try {
      Left(xabs.inject[Addr, Exp, Abs](x))
    } catch {
      case UnsupportedLatticeElement =>
        Right(yabs.inject[Addr, Exp, Abs](x))
    }
    ...
    def obj[Addr : Address](base: Addr) = try {
      Left(xabs.obj[Addr](base))
    } catch {
      case UnsupportedLatticeElement =>
        Right(yabs.obj[Addr](base))
    }    
  }
}
