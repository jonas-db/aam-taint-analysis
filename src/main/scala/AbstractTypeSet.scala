...
object AbstractTypeSet {
  ...
  case class AbstractSymbol(v: String) extends AbstractTypeSet {
  ...
  case class AbstractPrimitive[Addr : Address, Exp : Expression, Abs : AbstractValue](prim: Primitive[Addr, Exp, Abs]) extends AbstractTypeSet {
  ...
  case class AbstractObject[Addr : Address](base: Addr) extends AbstractTypeSet
  ...
  implicit object AbstractTypeSetAbstractValue extends AbstractValue[AbstractTypeSet] {
    ...
    def objects[Addr : Address](x: AbstractTypeSet) = x match {
      case AbstractObject(base: Addr) => Set(base)
      case AbstractSet(_) => x.foldValues(y => objects[Addr](y))
      case _ => Set()
    }    
    def symbols(x: AbstractTypeSet):Set[String] = x match {
      case AbstractSymbol(base: String) => Set(base)
      case AbstractSet(_) => x.foldValues(y => symbols(y))
      case _ => Set()
    }        
    ...
    private def toString[Addr : Address, Exp : Expression](x: AbstractTypeSet, store: Store[Addr, Exp, AbstractTypeSet], inside: Boolean, visited: Set[AbstractTypeSet]): String =
    ...
    def toString[Addr : Address, Exp : Expression](x: AbstractTypeSet, store: Store[Addr, Exp, AbstractTypeSet]) = toString(x, store, false, Set())
    ...
    def getPrimitive[Addr : Address, Exp : Expression, Abs : AbstractValue](x: A) = x match {
    ...
    def sanitize(x: AbstractTypeSet) = x
    def sink(x: AbstractTypeSet) = x
    def taint(x: AbstractTypeSet, sources:Set[Position]) = x
