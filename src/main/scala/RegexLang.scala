...
abstract class Regex {
 ...
 (
     r : Regex, 
     stack:List[(Regex => Regex)]=List(((a => Final(a)):(Regex => Regex)))
 ):Regex = r match
 {
   case Final(x) => x
...
//     r : Regex, 
//     stack:List[(Regex => Regex)]=List(((a => Final(a)):(Regex => Regex))),
//     cache:Map[Regex, Boolean]=Map[Regex, Boolean]()
// ):Regex = r match
// {
//   case Final(x) => x
...
//     x
//   }
//   case x:Event => simplifyr(stack.head(x), stack.tail,cache)
//   case x:EmptySet => simplifyr(stack.head(x), stack.tail,cache)
//   case x:EmptyWord => simplifyr(stack.head(x), stack.tail,cache)
//
//   case Star(x) => simplifyr(x, ((a => StarFrame(a)):(Regex => Regex)) :: stack,cache)
//   case StarFrame(x:EmptySet) => simplifyr(stack.head(EmptyWord()), stack.tail,cache)
//   case StarFrame(x:EmptyWord) => simplifyr(stack.head(EmptyWord()), stack.tail,cache)
//   case StarFrame(Event(x)) => simplifyr(stack.head(Event(x+"*")), stack.tail,cache)
//   // anything else of STAR
//   case StarFrame(x) => simplifyr(stack.head(Star(x)), stack.tail,cache + (Star(x) -> true))
//   
//   case Or(x, y) => simplifyr(x, ((a => OrFrame(a, y)):(Regex => Regex)) :: stack,cache)
//   case OrFrame(done, y) => simplifyr(y, ((a => OrMergeFrame(done, a)):(Regex => Regex)) :: stack, cache)
//   case OrMergeFrame(x:EmptySet, y) => simplifyr(stack.head(y), stack.tail, cache)
//   case OrMergeFrame(x, y:EmptySet) => simplifyr(stack.head(x), stack.tail, cache)
//   // TODO:NOT SURE?? but should be ok, because they mimic a transition to itself without consuming symbol
//   case OrMergeFrame(x:EmptyWord, y) => simplifyr(stack.head(y), stack.tail, cache)
//   case OrMergeFrame(x, y:EmptyWord) => simplifyr(stack.head(x), stack.tail, cache)
//   case OrMergeFrame(Event(x), Event(y)) if(x == y) => simplifyr(stack.head(Event(x)), stack.tail, cache) 
//   // anything else of OR, we could not simplify it, so we should cache it
//   case OrMergeFrame(x, y) => simplifyr(stack.head(Or(x, y)), stack.tail, cache + (Or(x, y) -> true))
//     
//   case Concat(x, y) => simplifyr(x, ((a => { AndFrame(a, y)}):(Regex => Regex)) :: stack, cache)
//   case AndFrame(done, y) => simplifyr(y, ((a => { AndMergeFrame(done, a)}):(Regex => Regex)) :: stack, cache)
//   case AndMergeFrame(x:Event, y:Event) => simplifyr(stack.head(Event(x + "," + y)), stack.tail, cache)
//   case AndMergeFrame(x, y:EmptySet) => simplifyr(stack.head(EmptySet()), stack.tail, cache)
//   case AndMergeFrame(x, y:EmptyWord) => simplifyr(stack.head(x), stack.tail, cache)
//   case AndMergeFrame(x:EmptySet, y) => simplifyr(stack.head(EmptySet()), stack.tail, cache)
//   case AndMergeFrame(x:EmptyWord, y) => simplifyr(stack.head(y), stack.tail, cache)
//   // a* . b => a*b
//   case AndMergeFrame(Star(a:Event), Event(b)) => simplifyr(stack.head(Event(a + "*," + b)), stack.tail, cache)
//   case AndMergeFrame(Event(b), Star(a:Event)) => simplifyr(stack.head(Event(b + "," + a+"*")), stack.tail, cache)
//   
//   // ((a+b) . (c+d)) = ( (ac + ad) + (bc + bd) )
//   case AndMergeFrame(Or(a, b), Or(c, d)) =>  
//     simplifyr(Or(Or(Concat(a,c), Concat(a,d)), Or(Concat(b,c), Concat(b,d)) ),
//         ((a => MulFrame(a)):(Regex => Regex)) :: stack, cache)
//   
//   
//   // c . (a + b) = ca + cb
//   case AndMergeFrame(c, Or(a, b)) => 
//     simplifyr(Or(Concat(c, a), Concat(c, b)), ((a => MulFrame(a)):(Regex => Regex)) :: stack, cache)
//   // (a + b) . c = ac + bc
//   case AndMergeFrame(Or(a, b), c) => 
//     simplifyr(Or(Concat(a, c), Concat(b, c)), ((a => MulFrame(a)):(Regex => Regex)) :: stack, cache)
//   case MulFrame(done) => simplifyr(stack.head(done), stack.tail, cache)
//
//   // anything else of AND
//   case AndMergeFrame(x, y) => simplifyr(stack.head(Concat(x, y)), stack.tail, cache + (Concat(x, y) -> true))
//   
// }
// 
...
//   case StarFrame(Event(x)) => simplifyr(stack.head(Event(x+"*")), stack.tail)
//   // anything else of STAR
//   case StarFrame(x) => simplifyr(stack.head(Star(x)), stack.tail)
//   
//   case Or(x, y) => simplifyr(x, ((a => OrFrame(a, y)):(Regex => Regex)) :: stack )
//   case OrFrame(done, y) => simplifyr(y, ((a => OrMergeFrame(done, a)):(Regex => Regex)) :: stack )
//   case OrMergeFrame(x:EmptySet, y) => simplifyr(stack.head(y), stack.tail)
...
//   case OrMergeFrame(Event(x), Event(y)) if(x == y) => simplifyr(stack.head(Event(x)), stack.tail) 
//   // anything else of OR, we could not simplify it, so we should cache it
//   case OrMergeFrame(x, y) => simplifyr(stack.head(Or(x, y)), stack.tail)
//     
//   case Concat(x, y) => simplifyr(x, ((a => { AndFrame(a, y)}):(Regex => Regex)) :: stack )
//   case AndFrame(done, y) => simplifyr(y, ((a => { AndMergeFrame(done, a)}):(Regex => Regex)) :: stack )
//   case AndMergeFrame(x:Event, y:Event) => simplifyr(stack.head(Event(x + "," + y)), stack.tail)
...
//
//   // anything else of AND
//   case AndMergeFrame(x, y) => simplifyr(stack.head(Concat(x, y)), stack.tail)
//   
// }
//  
...
// 
//  //@scala.annotation.tailrec
//  final def simplifyy(r : Regex):Regex = r match
// {
//   case Or(x, y:EmptySet) => simplify(x)
//   case Or(x:EmptySet, y) => simplify(y)
//   case Or(Event(x), Event(y)) => if(x == y) Event(x) else Or(Event(x), Event(y))
...
//   //added to fix (1), ie a:Star to string -> Event(..), this rule removes the extra event
//   case Star(Event(a)) => Event(a + "*")
//   case Star(Or(x:EmptyWord, y)) => Or(x, Star(y))
//   case Star(Or(y, x:EmptyWord)) => Or(x, Star(y))
//   case Concat(x, y:EmptySet) => EmptySet()
//   case Concat(x, y:EmptyWord) => simplify(x)
//   case Concat(x:EmptySet, y) => EmptySet()
//   case Concat(x:EmptyWord, y) => simplify(y)
...
//   case Concat(Star(a:Event), Event(b)) => Event(a + "*," + b)
//   // c* . (a + b) = c*a + c*b
//   case Concat(c:Star, Or(a, b)) => simplify(Or(Concat(c, a), Concat(c, b)))
//   // (a + b) . c* = ac* + bc*
//   case Concat(Or(a, b), c:Star) => simplify(Or(Concat(a, c), Concat(b, c)))
//   
//   // ((a+b) . (c+d)) = ( (ac + ad) + (bc + bd) )
//   case Concat(Or(a, b), Or(c, d)) => 
//     simplify(Or(Or(Concat(a, c), Concat(a, d)), Or(Concat(b, c), Concat(b, d))))
//   
//   // x . (a + b) = x.a + x.b
//   case Concat(x:Event, Or(a, b)) => simplify(Or(Concat(x, a), Concat(x, b)))
//   // (a + b) . x = a.x + b.x
//   case Concat(Or(a, b), x:Event) => simplify(Or(Concat(a, x), Concat(b, x)))
//   case Concat(Event(x), Event(y)) => Event(x + "," + y)  
//   
//   case _ => {
//     //println("matched nothing")
