...
object AbstractConcrete {
  ...
  case class AbstractPrimitive[Addr : Address, Exp : Expression, Abs : AbstractValue](prim: Primitive[Addr, Exp, Abs]) extends AbstractConcrete {
  ...
  case class AbstractObject[Addr : Address](base: Addr) extends AbstractConcrete 
  ...
  implicit object AbstractConcreteAbstractValue extends AbstractValue[AbstractConcrete] {
    ...
    def objects[Addr : Address](x: AbstractConcrete) = x match {
      case AbstractObject(base : Addr) => Set(base)
      case _ => Set()
    }
    private def toString[Addr : Address, Exp : Expression](x: AbstractConcrete, store: Store[Addr, Exp, AbstractConcrete], inside: Boolean, visited: Set[AbstractConcrete]): String =
    ...
    def toString[Addr : Address, Exp : Expression](x: AbstractConcrete, store: Store[Addr, Exp, AbstractConcrete]) = toString(x, store, false, Set())

    def symbols(x: AbstractConcrete):Set[String] = x match {
      case AbstractSymbol(s: String) => Set(s)
      case _ => Set()      
    }
    ...
    def getPrimitive[Addr : Address, Exp : Expression, Abs : AbstractValue](x: AbstractConcrete) = x match {
    ...
    def sanitize(x: AbstractConcrete) = x
    def sink(x: AbstractConcrete) = x
    def taint(x: AbstractConcrete, s:Set[Position]) = x
