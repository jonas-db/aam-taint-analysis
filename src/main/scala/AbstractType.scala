...
object AbstractType {
  ...
  case class AbstractPrimitive[Addr : Address, Exp : Expression, Abs : AbstractValue](prim: Primitive[Addr, Exp, Abs]) extends AbstractType {
  ...
  case class AbstractObject[Addr : Address](base: Addr) extends AbstractType
  ...
  implicit object AbstractTypeAbstractValue extends AbstractValue[AbstractType] {
    ...
    def objects[Addr : Address](x: AbstractType) = x match {
      case AbstractObject(base: Addr) => Set(base)
      case _ => Set()
    }    
    def symbols(x: AbstractType):Set[String] = ???
    
    private def toString[Addr : Address, Exp : Expression](x: AbstractType, store: Store[Addr, Exp, AbstractType], inside: Boolean): String = x match {
    ...
    def toString[Addr : Address, Exp : Expression](x: AbstractType, store: Store[Addr, Exp, AbstractType]) = toString(x, store, false)

    def getClosures[Exp : Expression, Addr : Address](x: AbstractType) = x match {
      case v: AbstractClosures[Exp, Addr] => v.clos
      case _ => Set()
    }
    def getPrimitive[Addr : Address, Exp : Expression, Abs : AbstractValue](x: AbstractType) = x match {
    ...
    def sanitize(x: AbstractType) = x
    def sink(x: AbstractType) = x
    def taint(x: AbstractType, sources:Set[Position]) = x
    def top = AbstractTop
