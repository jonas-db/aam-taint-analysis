...
class AAC[Exp : Expression, Abs : AbstractValue, Addr : Address, Time : Timestamp]
   ...
   */
  case class Context(clo: (Exp, Environment[Addr]), argsv: List[(Exp, Abs)], σ: Store[Addr, Exp, Abs]) {
   ...
   */
  case class State(control: Control, σ: Store[Addr, Exp, Abs], ι: LocalKont, κ: Kont, t: Time) {
    def this(exp: Exp) = this(ControlEval(exp, Environment.empty[Addr]().extend(primitives.forEnv)),
                               Store.initial[Addr, Exp, Abs](primitives.forStore), new LocalKont(), KontEmpty, time.initial)
  ...
  case class AACOutput(halted: Set[State], count: Int, t: Double, graph: Option[Graph[State, Unit]])
    ...
    def numberOfMusts = ???
     ...
     def shortestPaths = ???
//    def numberOfDistinctMays = ???
//    def numberOfDistinctMusts = ???
    def time = t
